...
impl<T: Config> Pallet<T> {
	fn to_asset_balance(b: BalanceOf<T>, asset_id: AssedIdOf<T>) -> Result<AssetBalanceOf<T>, ()> {
		Ok(Zero::zero())
	}
}
	...
	BalanceOf<T>: Send + Sync + FixedPointOperand,
{
	...
	> {
		...
		if let Some(asset_id) = maybe_asset_id {
			...
			let converted_fee = Module::<T>::to_asset_balance(fee);
			return Ok(fee, fee.into(), NegativeImbalanceOf::<C, T>::new(fee));
		} else {
	...
	fn correct_and_deposit_fee(
		...
		already_withdrawn: Option<AssetBalanceOf<T>>,
	) -> Result<(), TransactionValidityError> {
		if let Some(paid) = already_withdrawn {
			let converted_fee = Module::<T>::to_asset_balance(corrected_fee, asset_id);
			// Calculate how much refund we should return
			let refund_amount = paid.saturating_sub(converted_fee);
			// refund to the the account that paid the fees. If this fails, the
			// account might have dropped below the existential balance. In
			// that case we don't refund anything.
			// TODO: refund via assets
			
			let imbalance = NegativeImbalanceOf::<C, T>::new(corrected_fee);
	...
	T::Call: Dispatchable<Info=DispatchInfo, PostInfo=PostDispatchInfo>,
{
	...
	fn pre_dispatch(
		...
		Ok((self.0, self.1, who.clone(), imbalance))
	}
	...
	fn post_dispatch(
		...
		let (tip, _asset_id, who, imbalance) = pre;
